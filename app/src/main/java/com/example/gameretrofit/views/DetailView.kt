...
import com.example.gameretrofit.components.MainImage
import com.example.gameretrofit.components.MainTopBar
import com.example.gameretrofit.components.MetaWebSite
import com.example.gameretrofit.components.ReviewCard
...
fun DetailView(viewModel: GamesViewModel, navController: NavController, id: Int, name: String?) {
    ...
    Scaffold(
        topBar = {
            MainTopBar(title = viewModel.state.name, showBackButton = true, onClickBackButton = { navController.popBackStack() }) {}
        }
    ) {
...
fun ContentDetailView(pad: PaddingValues, viewModel: GamesViewModel) {
    ...
    Column(
        modifier = Modifier
            .padding(pad)
            .background(color = Color(CUSTOM_BLACK))
    ) {
